...
const AUTHENTIC_CATALOG = [
  ...
  // 6. KITCHEN
  {
    category: "Kitchen",
    items: [
      { brand: "Philips", name: "Philips Digital Air Fryer HD9252/90", price: 7999, specs: { Technology: "Rapid Air Convection Technology (90% Less Oil)", Capacity: "4.1 Liters", Presets: "7 Touchscreen Cooking Presets" }, image: "https://images.unsplash.com/photo-1556911220-e15b29be8c8f?w=800&auto=format&fit=crop&q=80" },
      { brand: "Instant Pot", name: "Instant Pot Duo 7-in-1 Multi-Cooker 6L", price: 8499, specs: { Functions: "Pressure Cooker, Slow Cooker, Rice Cooker, Steamer, Sauté, Yogurt Maker, Warmer", Safety: "10+ Proven Safety Mechanisms" }, image: "https://images.unsplash.com/photo-1544816155-12df9643f363?w=800&auto=format&fit=crop&q=80" },
      { brand: "Nespresso", name: "Nespresso Essenza Mini Espresso Coffee Machine", price: 14999, specs: { Pressure: "19-Bar High Pressure Pump", Heating: "25 Seconds Fast Heat-Up", Cups: "Espresso (40ml) & Lungo (110ml)" }, image: "https://images.unsplash.com/photo-1585515320310-259814833e62?w=800&auto=format&fit=crop&q=80" },
      { brand: "Bosch", name: "Bosch TrueMixx Pro 1000W Mixer Grinder", price: 7199, specs: { Motor: "1000W 3-Speed HiFlux Motor", Blades: "PoundingBlade for Authentic Dry Masalas", Jars: "4 Stainless Steel Jars" }, image: "https://images.unsplash.com/photo-1570222094114-d054a817e56b?w=800&auto=format&fit=crop&q=80" },
    ]
  },
...
function build1000AuthenticProducts() {
  const products = [];
  let prodIndex = 1;

  for (let i = 0; i < 1000; i++) {
    const catGroup = AUTHENTIC_CATALOG[i % AUTHENTIC_CATALOG.length];
    const template = catGroup.items[i % catGroup.items.length];
    const category = catGroup.category;
    const brand = template.brand;

    // Model variations for depth: storage, memory, generation variants
    const variantSuffix = ["", " (256GB Edition)", " (512GB High-Speed)", " (Special Edition)", " (Space Black)", " (Titanium Grey)"][(i % 6)];
    const fullName = `${template.name}${variantSuffix}`;

    const basePrice = template.price;
    const priceVariance = ((i % 5) - 2) * 500; // Small price step
    ...
    products.push({
      ...
      sku: `${brand.replace(/\s+/g, "").substring(0, 3).toUpperCase()}-${category.substring(0, 3).toUpperCase()}-${100 + (i % 900)}`,
      name: fullName,
      ...
      tags: [category.toLowerCase(), brand.toLowerCase(), "verified", "authentic"],
...
async function reseed() {
  ...
  const products = build1000AuthenticProducts();
  ...
  for (const dbName of targetDbs) {
    ...
    // Clear old scrambled catalog
    console.log(`Clearing old scrambled products in ${dbName}...`);
    await conn.collection("products").deleteMany({});

    console.log(`Inserting 1,000 authentic, brand-separated products into ${dbName}...`);
    await conn.collection("products").insertMany(products);

    const sample = await conn.collection("products").find({ category: "Phones" }).limit(5).toArray();
    console.log(`Sample Phones in ${dbName}:`);
    sample.forEach(p => console.log(`- [${p.specifications?.Brand}] ${p.name}`));
  ...
  console.log("\nSUCCESS! Catalog is now 100% brand-accurate. No brand cross-contamination!");
  process.exit(0);
}
