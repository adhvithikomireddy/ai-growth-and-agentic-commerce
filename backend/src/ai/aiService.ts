...
const generateDynamicSpecExplanation = (
  ...
  const recommendations = products.map((p) => {
    ...
    if (qLower.includes("code") || qLower.includes("programm") || qLower.includes("developer")) {
      reasonEn = `Selected because it packs ${proc ? `${proc} with ` : ""}${mem || "high-speed RAM"} and fast NVMe storage, ensuring smooth multi-threaded compilation and Docker workloads at ${priceStr}.`;
      reasonTe = `ప్రోగ్రామింగ్ మరియు కోడింగ్ అవసరాల కోసం ${proc ? `${proc} మరియు ` : ""}${mem || "16GB RAM"}తో మల్టీ-టాస్కింగ్ సామర్థ్యం అందిస్తూ ${priceStr} వద్ద సిఫార్సు చేయబడింది.`;
      reasonHi = `कोडिंग और सॉफ्टवेयर डेवलपमेंट के लिए ${proc ? `${proc} और ` : ""}${mem || "फास्ट रैम"} के साथ उत्कृष्ट परफॉरमेंस प्रदान करते हुए ${priceStr} में चुना गया है।`;
    } else if (qLower.includes("game") || qLower.includes("gaming")) {
      reasonEn = `Features high-bandwidth ${mem || "DDR5 memory"} and enhanced graphics cooling, delivering steady frame rates and low latency for modern gaming at ${priceStr}.`;
      reasonTe = `గేమింగ్ కోసం హై-స్పీడ్ మెమరీ మరియు మెరుగైన గ్రాఫిక్స్ పనితీరుతో స్థిరమైన ఫ్రేమ్‌రేట్‌లను అందిస్తూ ${priceStr} వద్ద ఎంపిక చేయబడింది.`;
      reasonHi = `गेमिंग के लिए हाई-स्पीड मेमोरी और बेहतर ग्राफिक्स कैपेबिलिटी के साथ स्थिर फ्रेम रेट्स प्रदान करते हुए ${priceStr} में चुना गया।`;
    } else if (qLower.includes("camera") || qLower.includes("photo") || qLower.includes("video")) {
      reasonEn = `Chosen for its high-grade optical sensor and dynamic range image processing, delivering crisp detail and color accuracy for content creation at ${priceStr}.`;
      reasonTe = `అద్భుతమైన ఫోటోగ్రఫీ మరియు వీడియో క్వాలిటీ కోసం అధునాతన ఆప్టికల్ సెన్సార్ మరియు ఇమేజ్ ప్రాసెసింగ్‌తో ${priceStr} వద్ద ఎంపిక చేయబడింది.`;
      reasonHi = `शानदार फोटोग्राफी और वीडियो रिकॉर्डिंग के लिए उच्च-गुणवत्ता वाले कैमरा सेंसर और क्लियर डिटेलिंग के साथ ${priceStr} में अनुशंसित किया गया।`;
    } else if (qLower.includes("quiet") || qLower.includes("silent") || qLower.includes("type") || qLower.includes("office")) {
      reasonEn = `Engineered with low-noise tactile switches and ergonomic contours, providing fatigue-free and quiet office typing productivity at ${priceStr}.`;
      reasonTe = `ఆఫీస్ టైపింగ్ మరియు రోజూవారీ ఉత్పాదకత కోసం తక్కువ శబ్దంతో కూడిన కీబోర్డ్ స్విచ్‌లు మరియు ఎర్గోనామిక్ డిజైన్‌తో ${priceStr} వద్ద ఎంపిక చేయబడింది.`;
      reasonHi = `ऑफिस टाइपिंग और दैनिक कार्य के लिए शांत की-स्ट्रोक्स और आरामदायक एर्गोनोमिक डिजाइन के साथ ${priceStr} में चुना गया है।`;
    } else if (qLower.includes("air fryer") || qLower.includes("bake") || qLower.includes("cook") || qLower.includes("kitchen")) {
      reasonEn = `Equipped with rapid air thermal convection for uniform low-fat cooking and multi-menu digital presets, verified at ${priceStr}.`;
      reasonTe = `ఆరోగ్యకరమైన తక్కువ కొవ్వు వంటకాల కోసం వేగవంతమైన ఎయిర్ సర్క్యులేషన్ మరియు డిజిటల్ టచ్ కంట్రోల్స్‌తో ${priceStr} వద్ద సిఫార్సు చేయబడింది.`;
      reasonHi = `हेल्दी और कम तेल में कुकिंग के लिए रैपिड एयर सर्कुलेशन टेक्नोलॉजी और डिजिटल प्री-सेट्स के साथ ${priceStr} में चुना गया।`;
    } else if (qLower.includes("drawing") || qLower.includes("tablet") || qLower.includes("art") || qLower.includes("sketch")) {
      reasonEn = `Features high-sensitivity pressure levels and a responsive stylus with zero lag, giving artists and students fluid digital sketching control at ${priceStr}.`;
      reasonTe = `డిజిటల్ ఆర్ట్ మరియు స్కెచింగ్ కోసం అధిక ప్రెజర్ సెన్సిటివిటీ మరియు ఖచ్చితమైన స్టైలస్ కంట్రోల్ అందిస్తూ ${priceStr} వద్ద ఎంపిక చేయబడింది.`;
      reasonHi = `डिजिटल आर्ट और स्केचिंग के लिए हाई प्रेशर सेंसिटिविटी और सटीक स्टाइलस कंट्रोल के साथ ${priceStr} में अनुशंसित किया गया।`;
    } else {
      reasonEn = `Authentic ${brand} ${p.category} featuring ${proc ? `${proc}, ` : ""}${mem ? `${mem}, ` : ""}${specs.Warranty || "official 1-year warranty"}, offering proven durability and certified stock at ${priceStr}.`;
      reasonTe = `అధికారిక ${brand} ${p.category} మోడల్ ${mem ? `${mem} ` : ""}మరియు తయారీదారుల వారంటీతో నమ్మకమైన పనితీరును అందిస్తూ ${priceStr} వద్ద ఎంపిక చేయబడింది.`;
      reasonHi = `प्रमाणित ${brand} ${p.category} जिसमें ${mem ? `${mem} ` : ""}और आधिकारिक निर्माता वारंटी शामिल है, ${priceStr} में विश्वसनीय विकल्प है।`;
    }
